...
mytoken = "xoxb-2005003311860-1998347017317-yBHh0HxyENwquqmD2NTzaSiU"
...
while True:
    try:
        ...
        if start_time < now < end_time - datetime.timedelta(seconds=10):
            X = ['KRW-MTL', 'KRW-XRP', 'KRW-ETC', 'KRW-OMG', 'KRW-SNT', 'KRW-WAVES', 'KRW-XEM', 'KRW-QTUM', 'KRW-LSK',
                 'KRW-STEEM', 'KRW-XLM', 'KRW-ARDR', 'KRW-KMD', 'KRW-ARK', 'KRW-STORJ', 'KRW-GRS', 'KRW-REP',
                 'KRW-EMC2', 'KRW-ADA', 'KRW-SBD', 'KRW-POWR', 'KRW-ICX', 'KRW-EOS', 'KRW-TRX', 'KRW-SC', 'KRW-IGNIS',
                 'KRW-ONT', 'KRW-ZIL', 'KRW-POLY', 'KRW-ZRX', 'KRW-LOOM', 'KRW-ADX', 'KRW-BAT', 'KRW-IOST', 'KRW-DMT',
                 'KRW-RFR', 'KRW-CVC', 'KRW-IQ', 'KRW-IOTA', 'KRW-MFT', 'KRW-ONG', 'KRW-GAS', 'KRW-UPP', 'KRW-ELF',
                 'KRW-KNC', 'KRW-THETA', 'KRW-EDR', 'KRW-QKC', 'KRW-BTT', 'KRW-MOC', 'KRW-ENJ', 'KRW-TFUEL', 'KRW-MANA',
                 'KRW-ANKR', 'KRW-AERGO', 'KRW-ATOM', 'KRW-TT', 'KRW-CRE', 'KRW-SOLVE', 'KRW-MBL', 'KRW-TSHP',
                 'KRW-WAXP', 'KRW-HBAR', 'KRW-MED', 'KRW-MLK', 'KRW-STPT', 'KRW-ORBS', 'KRW-VET', 'KRW-CHZ', 'KRW-PXL',
                 'KRW-STMX', 'KRW-DKA', 'KRW-HIVE', 'KRW-KAVA', 'KRW-AHT', 'KRW-LINK', 'KRW-XTZ', 'KRW-BORA', 'KRW-JST',
                 'KRW-CRO', 'KRW-TON', 'KRW-SXP', 'KRW-LAMB', 'KRW-HUNT', 'KRW-MARO', 'KRW-PLA', 'KRW-DOT', 'KRW-SRM',
        ...
        if True!= (start_time < now < end_time):
            target_price = get_target_price(X[i], 0.2)
            ...
            # 1분봉
            dd = pyupbit.get_ohlcv(X[i],"minute5")
            #10분 전 거래량
            firstVolume=int(dd.iloc[198]['volume'])
            #현재 거래량
            curVolume=int(dd.iloc[-1]['volume'])
            order = pyupbit.get_orderbook(X[1])
            orderbook = order[0]  # 리스트 내의 딕셔너리에 접근

            ask = orderbook['total_ask_size']  # 호가 매수 총 잔량
            bid = orderbook['total_bid_size']  # 호가 매도 총 잔량
            ...
            if ((current_price > target_price ) & (firstVolume*1.4 < curVolume) & (current_price > ma1) &(current_price > ma5) & (ask > bid*1.2)) & (500.0 < current_price < 60000.0):
                if krw > 5000.0:
                    buy_result = upbit.buy_market_order(X[i],krw*0.9995)
                    #구매가
                    buy = current_price
                    buy_result20 = buy * 1.025
                    buy_result09 = buy * 0.96
                    ...
                    post_message(mytoken, "#qqq", "5분전 거래량 : " + str(firstVolume))
                    post_message(mytoken, "#qqq", "현재 거래량 : " + str(curVolume))
                    post_message(mytoken, "#qqq", "매수 잔량(왼쪽) : " + str(ask))
                    post_message(mytoken, "#qqq", "매도 잔량(오른쪽) : " + str(bid))
                else:
                    btc = get_balance(Z[i])
                    if btc == None:
                        btc = 0
